 ...
 */
const inputHandlers = {
    'animation-type': updateAnimationType,
};
...
async function toggleAnimation() {
	try {
		innerCustomAnimationInput();
        pongCustomManager.toggleBackCustomManager();
		pongCustomAccessory.innerAccessory(6);
...
function toggleChangeInput(event) {
	...
	if (inputHandlers[id]) {
		...
		game.CustomAnimation.updateAnimation();
	} else {
 ...
 */
async function innerCustomAnimationInput() {
	try {
...
async function initializeAnimation() {
    try {
        if (game.CustomAnimation) {
            game.showAnimation();
            await updateCustomAnimationInput();
        } else {
            await game.createAnimation();
        }
    } catch {
        console.error('error in initializeAnimation');
    }
}
 ...
 */
async function updateCustomAnimationInput () {
	try {

	} catch {
		console.error('error in updateCustomAnimationInput')
	}
}
...
function updateAnimationType(value) {
    let options = ['option1', 'option2', 'option3'];
	if (pongCustomManager.checkSelect(value, options)) {
		// game.CustomAnimation.option = value;
    }
	else
